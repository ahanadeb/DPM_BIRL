...
def gen_trajectories(X, P, pi, tl):
    ...
    traj = np.zeros((tl, 2))
    next_s = 63 #random.randint(0, X - 1)  # initial state
    for i in range(0, tl):
        states = (np.arange(X)).reshape((X,))
        #next_s = random.choices(states, weights=P_pi[next_s, :].reshape((X,)), k=1)
        next_s = np.argmax(P_pi[next_s, :])
        traj[i, 0] = next_s
        traj[i, 1] = np.argmax(pi[next_s, :])
    return traj


def trajectory_set(F, RF,r,ch, traj_set_prev,rewards_prev,y_prev, tn, tl,drive):
    #r = get_reward(F, RF, drive) #shape RF*F 3*16
    ...
    x=0
    if ch!=0:
        for kl in range(0, traj_set_prev.shape[2]):
            traj_data[:, :, kl] = traj_set_prev[:, :, kl]
            x = x + 1
    for i in range(x, tn):
        j = int(y[i])
        seq.append(j)
